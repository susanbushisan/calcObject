...
public class AvgAggregateFunction extends SelfAggregateFunction {
    public AvgAggregateFunction(String name, String calcMeasureName) {
        super(name, calcMeasureName);
    }

    public AvgAggregateFunction(String name) {
        super(name);
    }
    ...
    public Function<List<Row>, CalcObject> getFunction() {
        return rows -> rows.stream().map(x->x.get(getCalcMeasureName())).reduce(CalcNull.instance(),CalcObject::add)
                .div(CalcDouble.valueOf(rows.size()));
    }
}
